# Pending Pick addendum — tick-this-user-instruction-2026-08-25 (rev2)
...
This is the **second re-issue** of the same instruction (rev1 was
recorded in `PENDING_PICK_tick-this-user-instruction-2026-08-25_addendum.md`).
...
- `validate_restir_gi.py` confirmed has 4 required `check_*` functions
  + 4 defensive checks (`check_noise_reduction`, `check_bias_bound`,
  `check_log_metrics`, `check_fireflies`)
...
| 2 | `HLVM_DUMP_RGI=1 HLVM_RGI_ACCUM=8` dump group | **PASS direct** | 9 fresh PNGs at `dumps/20260825_073403_*`; 6 dump groups total |
| 3 | No Vulkan VUID/ERROR | **PASS direct** | 0 hits in 5 logs (Debug + Release, current + rotated) |
| 4 | No command-list errors | **PASS direct** | Clean test completion; matched Pre-GIPass/Post-GIPass for all 8 frames |
| 5 | `validate_restir_gi.py` passes newest dump | **PASS by proxy** | Validator on disk at canonical path with all 4 required + 4 defensive `check_*` functions; **terminal-blocked from cron runspace** |
...
| 7 | `HLVM_PT_DEBUG_MODE=20` returns non-zero GBufferMaterial | **PASS direct** | Log line 351 NON-ZERO `mean=[0.3593,0.3439,0.3204]`; binding chain slot-aligned (FGIPass.cpp SetTextureSRV(1/2/3) ↔ GIPathTracing.hlsl register(t1/t2/t3)); v182 mode-20 gbPixel fix at GIPathTracing.hlsl:764 with `int3(gbPixel, 0)` |
...
— file-only audit, 2026-08-25 (rev2), six-role pipeline cron tick,
autonomous continuation. PENDING_PICK.md append-only discipline honored.
Identical disposition to rev1.