# Pipeline Health — 2026-07-30 scheduled tick 92

- Authoritative `docs/PENDING_PICK.md` is parent-evidence-gated: v126 is the top unchecked item; v127 is exhausted (file-only runspace) and v128 records the current blocked posture. The dispatcher must not start another file-only role cycle.
- Terminal capability probe was rejected before launch: `status=pending_approval`, `exit_code=-1`, `pattern_key=tirith::unknown` (consistent with 80+ inner v88-v123 cycles and 41+ outer ticks tick22→tick91).
