...
Build the target, run fresh `HLVM_DUMP_RGI=1 HLVM_RGI_ACCUM=8`, inspect appended logs only, isolate and validate the newest coherent frame-8 group, compute structural statistics, and directly inspect the display.

## Tests written
- None. v120 is verification-only and changed no production/test source.
