...
#include "Template/Obfuscate/String.tpp"
...
HLVM_INLINE_VAR const char* GPrivateKeyPKCS8 = OBFUSCATED_LONG("-----BEGIN PRIVATE KEY-----\n"
...
// load keypair
HLVM_INLINE_VAR Botan::secure_vector<uint8_t> in{ GPrivateKeyPKCS8, GPrivateKeyPKCS8 + std::strlen(GPrivateKeyPKCS8) };
// encrypt with pk
...
{
public:
	HLVM_STATIC_FUNC std::vector<uint8_t> EncryptPCKS8(const std::span<std::byte>& Buffer);
	HLVM_STATIC_FUNC Botan::secure_vector<uint8_t> DecryptPCKS8(const std::span<std::byte>& Buffer);
};
