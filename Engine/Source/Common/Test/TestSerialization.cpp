...
{
	return os << ' ' << gp.degrees << static_cast<unsigned char>(186) << gp.minutes << '\'' << gp.seconds << '"';
}
...
static void save_schedule(const bus_schedule& s, const char* filename)
{
	// make an archive
...
static void restore_schedule(bus_schedule& s, const char* filename)
{
	// open the archive
...
RECORD_INT(boost_serialization_test)
{
	// make the schedule
