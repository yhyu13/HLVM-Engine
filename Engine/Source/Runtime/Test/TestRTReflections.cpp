 ...
 *
 * RT Reflections Test - Textured Sponza GBuffer + Ray Traced Reflections
 *
 * Pipeline:
 ...
 * 3. RT Reflection Pass: Read GBuffer world position, cast reflection rays
 * 4. Blit Pass: Copy RT output to swapchain
 *
 * Mirror sphere at center, camera points at sphere for visible reflections.
 */
...
static const char*    WINDOW_TITLE = "RT Reflections Test";
...
class FRTReflectionsPass : public IRenderPass
{
public:
    ...
    {
        HLVM_LOG(LogTest, info, TXT("=== FRTReflectionsPass::Initialize ==="));
        ...
        const auto DataDir = FString::Format(
            TXT("{}/Engine/Source/Runtime/Test/TestRTReflections_Data"),
            *GProjectRoot);
        ...
        auto RTShaderBlob = ReadBinaryFile(
            FPath::Combine(DataDir, TXT("RTReflections.sblob")).string());
        ...
        // =====================================================================
        // Generate mirror sphere geometry
        // =====================================================================
        HLVM_LOG(LogTest, info, TXT("Generating mirror sphere..."));
        TVector<FVertex> SphereVertices;
        TVector<uint32_t> SphereIndices;
        {
            const float SphereRadius = 1.5f;
            const uint32_t SphereSegments = 32;
            const float PI = 3.14159265358979f;

            for (uint32_t i = 0; i <= SphereSegments; ++i)
            {
                float phi = PI * static_cast<float>(i) / SphereSegments;
                for (uint32_t j = 0; j <= SphereSegments; ++j)
                {
                    float theta = 2.0f * PI * static_cast<float>(j) / SphereSegments;
                    FVertex vert;
                    vert.Position = glm::vec3(
                        sinf(phi) * cosf(theta) * SphereRadius,
                        cosf(phi) * SphereRadius,
                        sinf(phi) * sinf(theta) * SphereRadius
                    );
                    vert.Normal = glm::normalize(vert.Position);
                    vert.UV = glm::vec2(static_cast<float>(j) / SphereSegments, static_cast<float>(i) / SphereSegments);
            ...
            for (uint32_t i = 0; i < SphereSegments; ++i)
            {
                for (uint32_t j = 0; j < SphereSegments; ++j)
                {
                    uint32_t row0 = i * (SphereSegments + 1) + j;
                    uint32_t row1 = row0 + SphereSegments + 1;
                    SphereIndices.push_back(row0);
                    SphereIndices.push_back(row1);
                    SphereIndices.push_back(row0 + 1);
                    SphereIndices.push_back(row1);
                    SphereIndices.push_back(row1 + 1);
                    SphereIndices.push_back(row0 + 1);
                }
            }
        }
        HLVM_LOG(LogTest, info, TXT("Generated sphere: {} vertices, {} indices"),
            SphereVertices.size(), SphereIndices.size());

// Create sphere vertex buffer with proper buffer state tracking
        {
            nvrhi::BufferDesc VBDesc;
            VBDesc.byteSize = SphereVertices.size() * sizeof(FVertex);
            VBDesc.isVertexBuffer = true;
            VBDesc.isVolatile = false;
            VBDesc.initialState = nvrhi::ResourceStates::CopyDest;
            VBDesc.debugName = "SphereVertexBuffer";
            SphereVertexBuffer = NvrhiDevice->createBuffer(VBDesc);

            nvrhi::CommandListHandle GeomCmdList = NvrhiDevice->createCommandList();
            GeomCmdList->open();
            GeomCmdList->beginTrackingBufferState(SphereVertexBuffer, nvrhi::ResourceStates::CopyDest);
            GeomCmdList->writeBuffer(SphereVertexBuffer, SphereVertices.data(), VBDesc.byteSize);
            GeomCmdList->setPermanentBufferState(SphereVertexBuffer, nvrhi::ResourceStates::VertexBuffer);
            GeomCmdList->close();
            NvrhiDevice->executeCommandList(GeomCmdList);
        }

        // Create sphere index buffer with proper buffer state tracking
        {
            nvrhi::BufferDesc IBDesc;
            IBDesc.byteSize = SphereIndices.size() * sizeof(uint32_t);
            IBDesc.isIndexBuffer = true;
            IBDesc.isVolatile = false;
            IBDesc.initialState = nvrhi::ResourceStates::CopyDest;
            IBDesc.debugName = "SphereIndexBuffer";
            SphereIndexBuffer = NvrhiDevice->createBuffer(IBDesc);

            nvrhi::CommandListHandle GeomCmdList = NvrhiDevice->createCommandList();
            GeomCmdList->open();
            GeomCmdList->beginTrackingBufferState(SphereIndexBuffer, nvrhi::ResourceStates::CopyDest);
            GeomCmdList->writeBuffer(SphereIndexBuffer, SphereIndices.data(), IBDesc.byteSize);
            GeomCmdList->setPermanentBufferState(SphereIndexBuffer, nvrhi::ResourceStates::IndexBuffer);
            GeomCmdList->close();
            NvrhiDevice->executeCommandList(GeomCmdList);
        }
        ...
        // Build sphere BLAS
        {
            FStaticMesh SphereMesh(TXT("MirrorSphere"), SphereVertices, SphereIndices);
            SphereBLAS = BLASBuilder::Build(NvrhiDevice, InitCmdList.Get(), SphereMesh);
            if (!SphereBLAS)
            {
                HLVM_LOG(LogTest, err, TXT("Failed to build sphere BLAS"));
                return false;
            }
        }
        HLVM_LOG(LogTest, info, TXT("Sphere BLAS built successfully"));

        // =====================================================================
        // Build TLAS
        // =====================================================================
        if (!TLASBuilder.Initialize(NvrhiDevice, static_cast<uint32_t>(StaticMeshes.size() + 1)))
        {
            HLVM_LOG(LogTest, err, TXT("Failed to initialize TLAS builder"));
            return false;
        }
        HLVM_LOG(LogTest, info, TXT("Building TLAS with {} instances (sphere + Sponza)..."), StaticMeshes.size() + 1);

        // Add sphere as instance 0 (at scene center, elevated slightly)
        {
            FTLASBuilder::FInstanceDesc SphereInstance;
            SphereInstance.BottomLevelAS = SphereBLAS;
            SphereInstance.InstanceMask = 1;
            SphereInstance.InstanceFlags = nvrhi::rt::InstanceFlags::TriangleFrontCounterclockwise;
            glm::mat4 SphereTransform = glm::translate(glm::mat4(1.0f), SceneCenter + glm::vec3(0.0f, 2.0f, 0.0f));
            SphereInstance.SetTransform(SphereTransform);
            if (!TLASBuilder.AddInstance(SphereInstance))
            {
                HLVM_LOG(LogTest, err, TXT("Failed to add sphere instance to TLAS"));
                return false;
            }
        }

        // Add Sponza meshes as instances 1, 2, ...
        ...
        HLVM_LOG(LogTest, info, TXT("FRTReflectionsPass initialized successfully"));
        return true;
    }

    void Shutdown()
    {
        HLVM_LOG(LogTest, info, TXT("FRTReflectionsPass::Shutdown"));
    ...
    virtual void Animate(float fElapsedTimeSeconds) override
    {
        ...
        {
            WindowTitle = FString::Format(TXT("RT Reflections - FPS: {:.1f}"), FPS);
    ...
    virtual void Render(nvrhi::IFramebuffer* Framebuffer) override
    {
        ...
        // =====================================================================
        // Static camera setup - points at mirror sphere
        // =====================================================================
        glm::vec3 SphereCenter = SceneCenter + glm::vec3(0.0f, 2.0f, 0.0f);
        glm::vec3 CameraPos = SphereCenter + glm::vec3(0.0f, 0.0f, 10.0f);
        glm::mat4 view = glm::lookAtLH(CameraPos, SphereCenter, glm::vec3(0.0f, 1.0f, 0.0f));
        ...
        float LightingData[16] = {
            ...
            CameraPos.x, CameraPos.y, CameraPos.z, 1.0f,    // CameraPos (w=1.0 for full reflection)
...
private:
    ...
    nvrhi::rt::AccelStructHandle          SphereBLAS;
    nvrhi::BufferHandle                  SphereVertexBuffer;
    nvrhi::BufferHandle                  SphereIndexBuffer;
...
RECORD_BOOL(test_RTReflections)
{
    HLVM_LOG(LogTest, info, TXT("=== Starting RT Reflections Test ==="));
    ...
    try
    {
        ...
        TSharedPtr<FRTReflectionsPass> RTPass =
            std::make_shared<FRTReflectionsPass>(DeviceManager.get());
        if (!RTPass->Initialize(NvrhiDevice, FirstFB, FString(TXT("RT Reflections Test"))))
        {
            throw std::runtime_error("Failed to initialize FRTReflectionsPass");
        }
