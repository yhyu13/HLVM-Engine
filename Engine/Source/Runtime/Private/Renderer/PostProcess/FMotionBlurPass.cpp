...
{
    bool FMotionBlurPass::Initialize(nvrhi::IDevice* InDevice, const FString& InShaderDataDir)
    {
        if (bIsInitialized)
        {
            Shutdown();
        ...
        HLVM_LOG(LogPostProcess, info, TXT("FMotionBlurPass::Initialize"));
        ...
        auto ShaderBlob = ReadBinaryFile(
            FPath::Combine(InShaderDataDir, TXT("MotionBlur_cs.sblob")).string());
        ...
        {
            HLVM_LOG(LogPostProcess, err, TXT("Failed to extract MotionBlur_cs from blob"));
            return false;
        }
        ...
        if (!ComputeShader)
        {
            HLVM_LOG(LogPostProcess, err, TXT("Failed to create MotionBlur_cs shader"));
            return false;
        }
        ...
        // Create binding layout
        {
            ...
            // b0 -> 256 (MotionBlurConstants)
            // t0 -> 0   (Color texture)
            // t1 -> 1   (Depth texture)
            // s0 -> 128 (Linear clamp sampler)
            // u0 -> 384 (Motion blur output)
            LayoutDesc.bindings = {
        ...
        // Create compute pipeline
        {
            ...
            if (!Pipeline)
            {
                HLVM_LOG(LogPostProcess, err, TXT("Failed to create motion blur compute pipeline"));
                return false;
            }
        }
        ...
        // Create constant buffer
        {
            ...
            BufferDesc.byteSize = sizeof(FMotionBlurConstants);
            BufferDesc.isConstantBuffer = true;
            BufferDesc.isVolatile = false;
            BufferDesc.keepInitialState = true;
            BufferDesc.initialState = nvrhi::ResourceStates::ConstantBuffer;
            BufferDesc.debugName = "MotionBlurConstants";
            ConstantBuffer = Device->createBuffer(BufferDesc);
        }

        HLVM_LOG(LogPostProcess, info, TXT("FMotionBlurPass initialized successfully"));
        bIsInitialized = true;
        return true;
    }

    void FMotionBlurPass::Dispatch(nvrhi::ICommandList* CmdList, const FDesc& Desc, const FMotionBlurConstants& Constants)
    {
        ...
        {
            HLVM_LOG(LogPostProcess, warn, TXT("FMotionBlurPass::Dispatch: invalid output dimensions"));
            return;
        }
        ...
        // VelocityScale
        ConstantsData[36] = Constants.VelocityScale;
        // MinVelocity
        ConstantsData[37] = Constants.MinVelocity;
    ...
    void FMotionBlurPass::Shutdown()
    {
        HLVM_LOG(LogPostProcess, info, TXT("FMotionBlurPass::Shutdown"));
